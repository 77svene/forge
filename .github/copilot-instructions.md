...
- `src/forge/` - Main package directory
...
- `src/train.py` - Training entry script (delegates to `forge.train.tuner`)
- `src/webui.py` - Web UI entry script (delegates to `forge.webui.interface`)
- `src/api.py` - API server entry script (delegates to `forge.api.app`)
- `tests/` - Test suite
...
- `src/forge/v1/` - Version 1 package directory
...
- Hyperparameters are defined using dataclasses in `src/forge/hparams/`

### Model Support

- New model support is added through model patches in `src/forge/model/`
- Visual models use the visual utilities in `src/forge/model/model_utils/visual.py`
- Quantization support is in `src/forge/model/model_utils/quantization.py`

### Data Processing

- Dataset definitions are in `data/dataset_info.json`
- Data templates and processors are in `src/forge/data/`

### Training

- Training pipelines are in `src/forge/train/`
...
- **CLI Training**: `forge-cli train --config examples/train_lora/llama3_lora_sft.yaml`
- **Web UI**: `forge-cli webui` or `python src/webui.py`
- **API Server**: `forge-cli api` or `python src/api.py`
- **Chat Interface**: `forge-cli chat --model_name_or_path MODEL_PATH`

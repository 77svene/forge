...
class ChatCompletionRequest(BaseModel):
    ...
    Extensions (non-OpenAI fields) are marked with 'x-forge'.
    """
    ...
    top_k: int = Field(20, ge = -1, le = 100, description = "[x-forge] Top-k sampling")
    min_p: float = Field(
        0.01, ge = 0.0, le = 1.0, description = "[x-forge] Min-p sampling threshold"
    )
    repetition_penalty: float = Field(
        1.1, ge = 1.0, le = 2.0, description = "[x-forge] Repetition penalty"
    )
    image_base64: Optional[str] = Field(
        None, description = "[x-forge] Base64-encoded image for vision models"
    )
    audio_base64: Optional[str] = Field(
        None, description = "[x-forge] Base64-encoded WAV for audio-input models (ASR)"
    )
    use_adapter: Optional[Union[bool, str]] = Field(
        None,
        description = (
            "[x-forge] Adapter control for compare mode. "
    ...
    enable_thinking: Optional[bool] = Field(
        None,
        description = "[x-forge] Enable/disable thinking/reasoning mode for supported models",
    )
    enable_tools: Optional[bool] = Field(
        None,
        description = "[x-forge] Enable tool calling for supported models",
    )
    enabled_tools: Optional[list[str]] = Field(
        None,
        description = "[x-forge] List of enabled tool names (e.g. ['web_search', 'python', 'terminal']). If None, all tools are enabled.",
    )
    auto_heal_tool_calls: Optional[bool] = Field(
        True,
        description = "[x-forge] Auto-detect and fix malformed tool calls from model output.",
    )
    max_tool_calls_per_message: Optional[int] = Field(
        10,
        ge = 0,
        description = "[x-forge] Maximum number of tool call iterations per message (0 = disabled, 9999 = unlimited).",
    )
    tool_call_timeout: Optional[int] = Field(
        300,
        ge = 1,
        description = "[x-forge] Timeout in seconds for each tool call execution (9999 = no limit).",
    )
    session_id: Optional[str] = Field(
        None,
        description = "[x-forge] Session/thread ID for scoping tool execution sandbox.",
    )

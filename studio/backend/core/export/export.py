...
from forge import FastLanguageModel, FastVisionModel
...
def _apply_wsl_sudo_patch():
    ...
    try:
        import forge_zoo.llama_cpp as llama_cpp_module
        ...
        logger.info(
            "Applied WSL sudo patch to " "forge_zoo.llama_cpp.do_we_need_sudo"
        )
...
This {model_type} model was trained 2x faster with [Unsloth](https://github.com/forgeai/forge) and Huggingface's TRL library.

[<img src="https://raw.githubusercontent.com/forgeai/forge/main/images/forge%20made%20with%20love.png" width="200"/>](https://github.com/forgeai/forge)
"""
...
class ExportBackend:
    ...
    def export_gguf(
        ...
        try:
            # Convert quantization method to lowercase for forge
            quant_method = quantization_method.lower()

            # Save locally if requested
            if save_directory:
                save_directory = str(resolve_export_dir(save_directory))
                # Resolve to absolute path so forge's relative-path internals
                ...
                # forge's convert_to_gguf writes output files relative to
                ...
                # forge saves intermediate HF model files into model_save_path.
                # forge-zoo's check_llama_cpp() uses ~/.forge/llama.cpp by default.

...
from forge import FastLanguageModel, FastVisionModel, is_bfloat16_supported
from forge.chat_templates import get_chat_template
...
class UnslothTrainer:
    ...
    def load_model(
        ...
        try:
            ...
            # the compiled cache. forge_compile_transformers() sets __UNSLOTH_PATCHED__
            # on each modeling module and replaces methods with exec'd code.
            # clear_forge_compiled_cache() deletes the disk cache, but the flag
            ...
            from utils.cache_cleanup import clear_forge_compiled_cache

            clear_forge_compiled_cache()
            ...
            if self._audio_type == "csm":
                ...
                #   "forge/Spark-TTS-0.5B"    (HF repo ID)
                from forge import FastModel
                ...
                if model_name.endswith("/LLM"):
                    ...
                    hf_repo = f"forge/{local_dir}"
                    llm_path = model_name
                else:
                    # "forge/Spark-TTS-0.5B" → local_dir="Spark-TTS-0.5B"
                    hf_repo = model_name
    ...
    def prepare_model_for_training(
        ...
        use_gradient_checkpointing: str = "forge",
        ...
        try:
            ...
            # Must be one of: True, False, or "forge"
            if isinstance(use_gradient_checkpointing, str):
                use_gradient_checkpointing = use_gradient_checkpointing.strip().lower()
                if (
                    use_gradient_checkpointing == ""
                    or use_gradient_checkpointing == "forge"
                ):
                    use_gradient_checkpointing = "forge"
                elif use_gradient_checkpointing in ("true", "1", "yes"):
                    use_gradient_checkpointing = True
                elif use_gradient_checkpointing in ("false", "0", "no"):
                    use_gradient_checkpointing = False
                else:
                    # Invalid value, default to "forge"
                    logger.warning(
                        f"Invalid gradient_checkpointing value: {use_gradient_checkpointing}, defaulting to 'forge'"
                    )
                    use_gradient_checkpointing = "forge"
            elif use_gradient_checkpointing not in (True, False, "forge"):
                # Invalid type or value, default to "forge"
                logger.warning(
                    f"Invalid gradient_checkpointing type/value: {use_gradient_checkpointing}, defaulting to 'forge'"
                )
                use_gradient_checkpointing = "forge"
    ...
    def _preprocess_bicodec_dataset(self, dataset, custom_format_mapping = None):
        ...
        # NOT in the forge/Spark-TTS-0.5B HF model repo. Clone it if needed.
        spark_code_dir = os.path.join(
            os.path.dirname(self._spark_tts_repo_dir), "Spark-TTS"
        )
    ...
    def start_training(
        ...
        wandb_project: str = "forge-training",
    ...
    def _train_worker(self, dataset: Dataset, **training_args):
        ...
        try:
            ...
            if training_args.get("enable_wandb", False) and training_args.get(
                ...
                wandb.init(
                    project = training_args.get("wandb_project", "forge-training")
                )
            ...
            if is_deepseek_ocr:
                ...
                if not _ensure_deepseek_ocr_installed():
                    error_msg = (
                        ...
                        "from huggingface_hub import snapshot_download; "
                        "snapshot_download('forge/DeepSeek-OCR', local_dir='deepseek_ocr')"
                    )
                    logger.error(error_msg)
                    self._update_progress(error = error_msg, is_training = False)
                    return
                ...
                from forge.trainer import UnslothVisionDataCollator
            ...
            if (
                ...
                try:
                    from forge.chat_templates import train_on_responses_only
    ...
    def _patch_adapter_config(self, output_dir: str) -> None:
        """Patch adapter_config.json with forge_training_method.
        ...
        try:
            ...
            config["forge_training_method"] = method
            logger.info(
                f"Patching adapter_config.json with forge_training_method='{method}'"
            )
...
def _ensure_deepseek_ocr_installed():
    ...
    try:
        ...
        snapshot_download(
            "forge/DeepSeek-OCR", local_dir = local_dir, local_dir_use_symlinks = False
        )

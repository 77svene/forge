...
def _bootstrap_studio_venv() -> None:
    ...
    """
    venv_lib = Path.home() / ".forge" / "studio" / ".venv" / "lib"
    if not venv_lib.exists():
        import warnings

        warnings.warn(
            f"Studio venv not found at {venv_lib.parent} -- run 'forge studio setup' first",
            stacklevel = 2,
        )
        return
...
def show_link(port: int = 8000):
    ...
    <div style="display: inline-block; padding: 20px; background: #ffffff; border: 2px solid #000000;
        ...
        <h2 style="color: #000000; margin: 0 0 12px 0; font-size: 26px; font-weight: 800;
            ...
            <img src="https://github.com/forgeai/forge/raw/main/studio/frontend/public/forge-gem.png"
